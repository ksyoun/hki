...
echo ========================================
echo   HKI v2 — Rundown
echo   Operador: https://localhost:8765/v2
...
echo.
python -c "from hki import config; from hki.server.app import _local_ip; ip=_local_ip(); p=config.PORT; s=config.public_scheme(); local=f'{s}://localhost:{p}/v2'; join=config.audience_join_url(ip); direct=config.captions_public_url(ip); print(f'Operador v2 (PC):    {local}'); print(f'QR primera vez:      {join}'); print(f'QR directo:          {direct}' if config.is_https() else f'Subtitulos:          {direct}'); (config.is_https() and print(f'  Guia HTTP :{config.HTTP_GUIDE_PORT} -> Continuar -> certificado en :{p}')); print('HTTPS: activo - Wake Lock + guia HTTP puerto '+str(config.HTTP_GUIDE_PORT) if config.is_https() else 'HTTP - para Wake Lock: HKI_HTTPS=true y python -m hki gen-cert'); print()"
echo.

start /b python -c "import time,webbrowser; from hki import config; time.sleep(2); webbrowser.open(f'{config.public_scheme()}://localhost:{config.PORT}/v2')"
